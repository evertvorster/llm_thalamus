#!/usr/bin/env python3
"""
llm-thalamus – single-threaded controller / message router.
...
from memory_retrieval import query_memories  # retrieval side:contentReference[oaicite:4]{index=4}
from memory_storage import store_semantic    # storage side:contentReference[oaicite:5]{index=5}
...
class ThalamusConfig:
    ...
    def load(cls, explicit_path: Optional[Path] = None) -> "ThalamusConfig":
        """
        Load configuration from config/config.json and fill in reasonable defaults.
        Shared with memory_* modules so everything points to the same OpenMemory
        and embedding/LLM configuration.
        """
        path = explicit_path or CONFIG_PATH
        if not path.exists():
            # Minimal fully-default config
            return cls()
        ...
        # Resolve prompt paths (relative to BASE_DIR unless absolute)
        ...
        # LLM URL: reuse embeddings.ollama_url by default
...
class PromptManager:
    """
    Loads plain-text prompt templates from disk.

    The goal is to keep llm-thalamus small by moving most prompt
    text into simple text files under config/. If a file is missing,
    we fall back to a short built-in default.
    """
    ...
    def get(self, name: str) -> str:
        ...
        # Built-in fallback prompts (kept deliberately short)
        if name == "answer":
            text = (
...
class ThalamusEvents:
    """
    Callback hooks that a UI (or other front-ends) can subscribe to.

    The UI implements these signatures in MainWindow:
      - handle_chat_message_event(role, content)
      - handle_status_update_event(subsystem, status, detail=None)
      - handle_thalamus_control_entry_event(label, raw_text)
    ...
    def __init__(self) -> None:
        ...
        self.on_status_update: Optional[
            Callable[[str, str, Optional[str]], None]
        ] = None
        self.on_thalamus_control_entry: Optional[
...
# ---------------------------------------------------------------------------
# Ollama client (blocking, single-threaded)
# ---------------------------------------------------------------------------


class OllamaClient:
    """
    Minimal blocking client for Ollama's /api/chat endpoint.
    """
...
class MemoryModule:
    """
    Thin wrapper around memory_retrieval / memory_storage.

    Keeps thalamus small and makes it easy to evolve memory behaviour
    later (e.g. use retrieval recipes, document retrieval, etc.).
    """
...
class Thalamus:
    """
    Single-threaded controller / router.

    One main public method drives a "session":

      process_user_message(user_message: str) -> str

    It:
      1. Starts a session (ID, events).
      2. Retrieves relevant memories.
      3. Calls the LLM to produce an answer.
      4. Emits the assistant answer to the UI, unchanged.
      5. Optionally runs a reflection call and stores new memories.
      6. Ends the session.

    Everything is synchronous; extensions (Stable Diffusion, STT/TTS, etc.)
    can be added as new modules without changing this structure.
    """
    ...
    def __init__(self, config: Optional[ThalamusConfig] = None) -> None:
        ...
        # Let the UI know we exist and are ready
    ...
    def process_user_message(self, user_message: str) -> str:
        ...
        Intended to be called by the UI when the user presses 'Send'.
        """
        ...
        # 1) Retrieve memories for this message
        ...
        # 3) Reflection call (optional, synchronous for now)
    ...
    def _debug_log(self, session_id: str, label: str, text: str) -> None:
        """
        Emit a control entry for the UI and log at DEBUG level.
        """
    ...
    def _call_llm_answer(
        ...
        """
        Build and send the answer-generation call.

        System prompt text comes from config/prompt_answer.txt (or a fallback).
        Dynamic context (memories, previous/current message) is sent as a user
        message so the template file can remain generic.
        """
    ...
    def _call_llm_reflection(
        ...
        """
        Build and send the reflection call.

        System prompt text comes from config/prompt_reflection.txt (or fallback).
        """
...
# ---------------------------------------------------------------------------
# Simple CLI entrypoint (for testing without UI)
# ---------------------------------------------------------------------------
...
def main(argv: Optional[List[str]] = None) -> int:
    _ = argv
    th = Thalamus()
    print("llm-thalamus CLI – type messages, Ctrl+C to exit.\n")
