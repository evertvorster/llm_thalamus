#!/usr/bin/env python3
"""
llm-thalamus UI – PySide6 implementation, wired to the Thalamus controller.

Based on the initial UI design and implementation, extended to:
- Load llm-thalamus.py dynamically.
- Connect Thalamus event callbacks to the UI.
- Send user messages to Thalamus when 'Send' is pressed.
"""
...
THALAMUS_PATH = BASE_DIR / "llm-thalamus.py"  # dynamically loaded
...
def load_config():
    ...
    if CONFIG_PATH.exists():
        try:
            ...
            pass
    # Default config – keep any new sections minimal but present
    cfg = {
        "logging": {
...
def save_config(cfg: dict):
    ...
    try:
        ...
        # For now we fail silently; Thalamus backend has its own logging.
        pass
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, config: dict, parent=None):
        ...
        self.thalamus = None  # will be initialised below
        ...
        # Load previous session chat (if configured)
        ...
        # Try to auto-connect to Thalamus
        if self.config.get("ui", {}).get("auto_connect_thalamus", True):
            self._init_thalamus()

        # Enable send only when thalamus is connected
    ...
    def _init_thalamus(self):
        """
        Dynamically load llm-thalamus.py and create a Thalamus instance.

        This allows the filename to contain a hyphen while still being importable.
        """
        if not THALAMUS_PATH.exists():
            self._append_thalamus_text(
                f"Thalamus script not found at {THALAMUS_PATH}. "
                "Send will remain disabled."
            )
            self.thalamus = None
            return
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                "llm_thalamus_module", THALAMUS_PATH
            )
            if spec is None or spec.loader is None:
                raise ImportError("Failed to create import spec for llm-thalamus.py")

            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            ThalamusClass = getattr(module, "Thalamus", None)
            if ThalamusClass is None:
                raise AttributeError("Thalamus class not found in llm-thalamus.py")
            ...
            # LLM / Memory will update themselves as Thalamus emits status events
            self._update_send_enabled(True)
        except Exception as e:
            self._append_thalamus_text(
                f"Failed to initialise Thalamus: {e}\n"
                "Check that llm-thalamus.py exists and imports cleanly."
            )
            self.thalamus = None
    ...
    def restart_thalamus(self):
        """
        For now, just re-initialise the Thalamus instance in-process.
        """
        if self.thalamus is not None:
            # Drop reference; there are no background threads
            self.thalamus = None
    ...
    def _on_view_mode_changed(self):
        # Raw vs rendered is a no-op for now.
        pass
    ...
    def _on_send_clicked(self):
        ...
        # Thalamus will emit the "user" chat event, so we do not write here.
        self.chat_input.clear()
        try:
            # This call is blocking; in future we might move it to a worker thread.
    ...
    # These methods are called by Thalamus event callbacks:
