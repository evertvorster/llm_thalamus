...
Usage: python3 sdxl_gen.py [--model-path PATH] [--out-dir DIR] <prompt>
"""
import argparse
import sys
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-path", default="/home/evert/models/sdxl-base",
                        help="Path to SDXL model directory")
...
def main() -> None:
    ...
    if not prompt:
        print("Usage: python3 sdxl_gen.py [--model-path PATH] [--out-dir DIR] <prompt>", file=sys.stderr)
        sys.exit(1)
    ...
    out_path = f"{args.out_dir.rstrip('/')}/sdxl_{stamp}.png"
