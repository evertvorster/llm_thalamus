...
class ControllerWorker(QObject):
    ...
    def _handle_message(self, text: str) -> None:
        ...
        try:
            ...
            # Option A: Always refresh the worker's authoritative world snapshot from the
            # graph's in-memory world before reflect runs. This prevents reflect commits
            # (topics-only) from accidentally writing stale structural fields back to disk.
            try:
                ...
                if isinstance(w_any, dict) and all(
                    ...
                    self.log_line.emit("[world] refreshed from state.world (pre-reflect)")
            except Exception as e:
