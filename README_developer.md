...
| reflect_memory | nodes/llm_reflect_memory.py |
...
Note: There is no pytest configuration (no pytest.ini, conftest.py, or pyproject.toml pytest section). Tests can be run with python -m pytest src/tests/ but lack discovery/fixture configuration.

Note: There is no pyproject.toml or requirements.txt -- the project has no build system or dependency pinning configured.
