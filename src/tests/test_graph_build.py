...
def test_default_graph_path_uses_primary_agent_then_split_reflect() -> None:
    ...
    assert "reflect_topics" in node_ids
    assert "reflect_memory" in node_ids
    ...
    assert ("primary_agent", "reflect_topics", False) in edges
    assert ("reflect_topics", "reflect_memory", False) in edges
    assert ("reflect_memory", "__end__", False) in edges

    assert ("context_bootstrap", "context_builder", False) not in edges
    assert ("context_builder", "answer", True) not in edges
    assert ("answer", "reflect_topics", False) not in edges
